    ...
    internal class Program
    {
        ...
        static void Main(string[] args)
        {
            try
            {
                ...
                using (var client = new HttpClient())
                {
                    ...
                    client.BaseAddress = new Uri(BASEADDR_URL_FLORENCE);

                    // Post the request to the web service
                    var response = client.GetAsync($"{BASEADDR_URL_FLORENCE}device").GetAwaiter().GetResult();
        ...
        static Task TestMYDL(string[] args)
        {
            ...
            return Task.Run(() =>
            {
                foreach (string imageFileName in args)
                {
                    try
                    {
                        ScanMYDLResult scanMYDLResult = ScanIDOCR.ScanMYDL(BASEADDR_URL_FLORENCE, imageFileName);
        ...
        static Task TestMyKad(string[] args)
        {
            ...
            return Task.Run(() =>
            {
                foreach (string imageFileName in args)
                {
                    try
                    {
                        ScanMyKadResult scanMyKadResult = ScanIDOCR.ScanMyKad(BASEADDR_URL_FLORENCE, imageFileName);
        ...
        static Task TestPHUMID(string[] args)
        {
            ...
            return Task.Run(() =>
            {
                foreach (string imageFileName in args)
                {
                    try
                    {
                        ScanPHUMIDResult scanPHUMIDResult = ScanIDOCR.ScanPHUMID(BASEADDR_URL_FLORENCE, imageFileName, BASEADDR_URL_FLORENCE);
        ...
        static string PostOCRRequest(string b64Image)
        {
            ...
            using (var client = new HttpClient())
            {
                ...
                DateTime dtStart = DateTime.Now;
                var response = client.PostAsync($"{BASEADDR_URL_FLORENCE}ocr", content).GetAwaiter().GetResult();
                DateTime dtEnd = DateTime.Now;
