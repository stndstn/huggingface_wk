...
MatchTemplateIDCard? matchTemplate = null;
// get module path 
...
foreach (JObject jObject in jArray)
{
    ...
    string? strPathCurDir = System.IO.Directory.GetCurrentDirectory();
    if (strPathCurDir == null)
    {
        Console.WriteLine($"Current working directory not found.");
        continue;
    }

    // find template folder in current working directory
    //string strPathTmplDir = Path.Combine(strPathCurDir, "tmpl");
    DirectoryInfo? diTmpl = null;
    if (Directory.Exists(strPathTmplDir))
    {
        diTmpl = new DirectoryInfo(strPathTmplDir);
    }

    TestImageFile(fiImage, fiImageBack, fiData, diTmpl);
...
#else
void TestImageFile(FileInfo fiImage, FileInfo fiImageBack, FileInfo fiData, DirectoryInfo? diTmpl = null)
{
    ...
    switch (expectedCountry)
    {
        case "MY":
            ...
            {
                Console.WriteLine($"calling OCRIDImageOSS BASEADDR_URL_OMNI_PARSER...");
                OCRIDImageOSS(ScanIDOCR.Create(strPathTmplDir, "MY"), BASEADDR_URL_OMNI_PARSER, expectedCountry, expectedDocumentType, fiImage, fiImageBack, expectedDataJson);
            }
            if (!string.IsNullOrEmpty(BASEADDR_URL_FLORENCE))
            {
                Console.WriteLine($"calling OCRIDImageOSS BASEADDR_URL_FLORENCE...");
                OCRIDImageOSS(ScanIDOCR.Create(strPathTmplDir, "MY"), BASEADDR_URL_FLORENCE, expectedCountry, expectedDocumentType, fiImage, fiImageBack, expectedDataJson);
            }            
            OCRIDImageByAzure(ScanIDOCR.Create(strPathTmplDir, "MY"), fiImage, fiImageBack, expectedDataJson);
        ...
        case "PH":
            ...
            {
                Console.WriteLine($"calling OCRIDImageOSS BASEADDR_URL_OMNI_PARSER...");
                OCRIDImageOSS(ScanIDOCR.Create(strPathTmplDir, "PH"), BASEADDR_URL_OMNI_PARSER, expectedCountry, expectedDocumentType, fiImage, fiImageBack, expectedDataJson);
            }
            if (!string.IsNullOrEmpty(BASEADDR_URL_FLORENCE))
            {
                Console.WriteLine($"calling OCRIDImageOSS BASEADDR_URL_FLORENCE...");
                OCRIDImageOSS(ScanIDOCR.Create(strPathTmplDir, "PH"), BASEADDR_URL_FLORENCE, expectedCountry, expectedDocumentType, fiImage, fiImageBack, expectedDataJson);
            }
            OCRIDImageByAzure(ScanIDOCR.Create(strPathTmplDir, "PH"), fiImage, fiImageBack, expectedDataJson);
    ...
    ScanIDOCR.ReleaseStaticResources();
}
#endif

void OCRIDImageOSS(ScanIDOCR scanIDOCR, string baseUrlAddress, string expectedCountry, string expectedDocumentType, FileInfo fiImage, FileInfo? fiImageBack, string expectedDataJson)
{
    if (scanIDOCR != null)
    {
        try
        {
            ScanIDResult scanIDResult = scanIDOCR.ScanID(baseUrlAddress, fiImage.FullName, fiImageBack?.FullName);
            switch (expectedCountry)
            {
                case "MY":
                    switch (expectedDocumentType)
                    {
                        case "DL":
                            try
                            {
                                ScanMYDLResult scanMYDLResult = scanIDResult as ScanMYDLResult;
                                if (scanMYDLResult != null)
                                {
                                    ...
                                    bool isValid = IsScanMYDLResultValid(scanMYDLResult, dataExpected);
                                    Console.WriteLine(" --> IsValid: " + isValid);
                                }
                                else
                                {
                                    Console.WriteLine($"scanIDResult.Success: {scanIDResult.Success}");
                                    Console.WriteLine($"scanIDResult.Error: {scanIDResult.Error}");
                                }
                            }
                            catch (Exception ex)
                            {
                        ...
                        case "MY":
                            try
                            {
                                ScanMyKadResult scanMyKadResult = scanIDResult as ScanMyKadResult;
                                if (scanMyKadResult != null)
                                {
                                    ...
                                    bool isValid = IsScanMyKadResultValid(scanMyKadResult, dataExpected);
                                    Console.WriteLine(" --> IsValid: " + isValid);
                                }
                                else
                                {
                ...
                case "PH":
                    switch (expectedDocumentType)
                    {
                        case "UI":
                            try
                            {
                                ScanPHUMIDResult scanPHUMIDResult = scanIDResult as ScanPHUMIDResult;
                                if (scanPHUMIDResult != null)
                                {
                                    ...
                                    bool isValid = IsScanPHUMIDResultValid(scanPHUMIDResult, dataExpected);
                                    Console.WriteLine(" --> IsValid: " + isValid);
                                }
                                else
                                {
                        ...
                        case "DL":
                            try
                            {
                                ScanPHDLResult scanPHDLResult = scanIDResult as ScanPHDLResult;
                                if (scanPHDLResult != null)
                                {
                                    ...
                                    bool isValid = IsScanPHDLResultValid(scanPHDLResult, dataExpected);
                                    Console.WriteLine(" --> IsValid: " + isValid);
                                }
                                else
                                {
                        ...
                        case "NI":
                            try
                            {
                                ScanPHNIResult scanPHNIResult = scanIDResult as ScanPHNIResult;
                                if (scanPHNIResult != null)
                                {
                                    ...
                                    Console.WriteLine($"scanPHNIResult.maritalStatus: {scanPHNIResult.maritalStatus}");
                                    ScanPHNIResult? dataExpected = JsonSerializer.Deserialize<ScanPHNIResult>(expectedDataJson, jsonOptions);
                                    System.Diagnostics.Debug.WriteLine("scanPHNIResult: " + scanPHNIResult);
                                    bool isValid = IsScanPHNIResultValid(scanPHNIResult, dataExpected);
                                    Console.WriteLine(" --> IsValid: " + isValid);
                                }
                                else
                                {
        ...
        {
            Console.WriteLine($"ScanID exception: {ex.Message}");
        }
    }
...
ScanIDResult? OCRIDImageByAzure(ScanIDOCR scanIDOCR, FileInfo fiImage, FileInfo fiImageBack, string expectedDataJson)
{
    /////////////
    bool isValid = false;
    ...
    ScanIDResult? result = ScanID.AzureVisionLib.GetScanResultAsync(fileImageBytes, fileImageBackBytes, scanIDOCR).GetAwaiter().GetResult();
    System.Diagnostics.Debug.WriteLine("ScanIDResult: " + result);
    if(result != null)
    {
        System.Diagnostics.Debug.WriteLine("ScanIDResult country: " + result.country);
        System.Diagnostics.Debug.WriteLine("ScanIDResult documentType: " + result.documentType);
                ...
                case "DL":
                    {
                        ...
                        if (dataExpected != null)
                        {
                            ScanMYDLResult? resultMYDL = result as ScanMYDLResult;
                            System.Diagnostics.Debug.WriteLine("ScanMYDLResult: " + resultMYDL);
                            if(resultMYDL != null)
                            {
                                isValid = IsScanMYDLResultValid(resultMYDL, dataExpected);
                                Console.WriteLine(" --> IsValid: " + isValid);
                            }
                        }
                        else
                ...
                case "MY":
                    {
                        ...
                        if (dataExpected != null)
                        {
                            ScanMyKadResult? resultMyKad = result as ScanMyKadResult;
                            System.Diagnostics.Debug.WriteLine("ScanMyKadResult: " + resultMyKad);
                            if (resultMyKad != null)
                            {
                                isValid = IsScanMyKadResultValid(resultMyKad, dataExpected);
                                Console.WriteLine(" --> IsValid: " + isValid);
                            }
                        }
                        else
                ...
                case "DL":
                    {
                        ...
                        if (dataExpected != null)
                        {
                            ScanPHDLResult? resultPHDL = result as ScanPHDLResult;
                            System.Diagnostics.Debug.WriteLine("ScanPHDLResult: " + resultPHDL);
                            if (resultPHDL != null)
                            {
                                isValid = IsScanPHDLResultValid(resultPHDL, dataExpected);
                                Console.WriteLine(" --> IsValid: " + isValid);
                            }
                        }
                        else
                ...
                case "UI":
                    {
                        ...
                        if (dataExpected != null)
                        {
                            ScanPHUMIDResult? resultPHUMID = result as ScanPHUMIDResult;
                            System.Diagnostics.Debug.WriteLine("ScanPHUMIDResult: " + resultPHUMID);
                            if (resultPHUMID != null)
                            {
                                isValid = IsScanPHUMIDResultValid(resultPHUMID, dataExpected);
                                Console.WriteLine(" --> IsValid: " + isValid);
                            }
                        }
                        else
                ...
                case "NI":
                    {
                        ...
                        if (dataExpected != null)
                        {
                            ScanPHNIResult? resultPHNI = result as ScanPHNIResult;
                            System.Diagnostics.Debug.WriteLine("ScanPHNIResult: " + resultPHNI);
                            if (resultPHNI != null)
                            {
                                isValid = IsScanPHNIResultValid(resultPHNI, dataExpected);
                                Console.WriteLine(" --> IsValid: " + isValid);
                            }
                        }
                        else
...
bool IsScanMyKadResultValid(ScanMyKadResult result, ScanMyKadResult expected)
{
    ...
    string genderExpected = expected.gender?.Trim();
    switch (genderExpected)
    {
        case "LELAKI":
            genderExpected = "M";
            break;
    ...
    if(result.gender?.Trim() != genderExpected)
    {
        countNotMatch++;
        Console.WriteLine($"\n[{countNotMatch}]");
        Console.WriteLine($"  Scanned Data: gender: {result.gender}");
        Console.WriteLine($"  Expected    : gender: {genderExpected} ({expected.gender})");
    }
...
bool IsScanPHNIResultValid(ScanPHNIResult result, ScanPHNIResult expected)
{
    ...
    if (result.maritalStatus?.Trim() != expected.maritalStatus?.Trim())
    {
        countNotMatch++;
        Console.WriteLine($"\n[{countNotMatch}]");
        Console.WriteLine("  Scanned Data: maritalStatus: " + result.maritalStatus);
        Console.WriteLine("  Expected    : maritalStatus: " + expected.maritalStatus);
    }
